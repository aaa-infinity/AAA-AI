// Fresh, from-scratch design system for the AAA App Store.
// A clean glassmorphism + gradient aesthetic with a real token system,
// animated hero, app cards, and a polished detail/login/profile experience.
// Consumed by storeShared.js (storeShell/page builders) so both workers
// (aaa-ai-bot + aaa-store) render the same redesigned site.

const C = {
  // Brand gradient stops reused across the UI.
  grad: "linear-gradient(135deg,#7c4dff 0%,#b14dff 45%,#ff4d9d 100%)",
  gradSoft: "linear-gradient(135deg,rgba(124,77,255,.18),rgba(255,77,157,.16))",
};

// ---------------------------------------------------------------------------
// Shell: <head> with token-driven CSS + persistent nav + footer.
// ---------------------------------------------------------------------------
...
export function shellNav(user, opts) {
  ...
  const chip = user
    ? '<a class="chip" href="/store/me" title="Your profile">' +
      (user.photo_url
        ? '<img class="av" src="' + esc(user.photo_url) + '" alt="">'
        : '<span class="av av--i">' + esc((user.display_name || user.tg_username || "U").slice(0, 1).toUpperCase()) + '</span>') +
      '<span class="chip-name">' + esc(user.display_name || user.tg_username || user.uid) + (user.is_premium ? ' <span class="star">★</span>' : '') + '</span></a>'
    : '';
  return '<nav class="nav">' +
    '<a class="brand" href="/store">' +
    '<span class="logo">A</span><span class="brand-tx">AA<span class="g">Store</span></span></a>' +
    '<div class="nav-act">' +
    '<button class="icn" onclick="toggleTheme()" title="Theme" aria-label="Toggle theme">◐</button>' +
    ...
    (user
      ? '<a class="pill" href="/store/upload">+ Publish</a>'
      : '<a class="pill" href="/store/login?next=' + next + '">Sign in</a>') +
    chip +
    (user ? '<button class="icn" onclick="logout()" title="Sign out" aria-label="Sign out">⏻</button>' : '') +
    '</div></nav>';
}
...
export function shellFoot() {
  return '<footer class="foot">' +
    ...
    '<a href="/store">Browse</a><a href="/download">Get the app</a><a href="/store/login">Sign in</a>' +
    '<a href="https://www.facebook.com/share/1BzWH5P2bF/" target="_blank" rel="noopener">Community</a>' +
    '</div></div>' +
    '<p class="copy">© ' + new Date().getFullYear() + ' AAA App Store · Built for the open Android community.</p>' +
    '</footer>';
}
...
export function shell(title, body, user, opts) {
  return shellHead(title) +
    ...
    'function logout(){try{var t=localStorage.getItem("sess");if(t){fetch("/api/store/logout",{method:"POST",headers:{"x-session":t}}).catch(function(){});}localStorage.removeItem("sess");localStorage.removeItem("sessUser");}catch(e){}location.href="/store";}' +
    '</script></body></html>';
}
...
export function homePage(list, categories, user) {
  const chips = ['<button class="cat ' + (!list.q && !list.category ? 'on' : '') + '" onclick="go(\'/store\')">All</button>']
    .concat(categories.map((c) =>
      '<button class="cat ' + (list.category === c ? 'on' : '') + '" onclick="go(\'/store?category=' + enc(c) + '\')">' + esc(c) + '</button>'
    )).join('');
  const grid = list.apps.length
    ? '<div class="grid">' + list.apps.map(appCard).join('') + '</div>'
  ...
  const body =
    ...
    '<div class="hero-cta">' +
    '<a class="btn" href="' + (user ? '/store/upload' : '/store/login?next=' + enc('/store/upload')) + '">⬆ Publish your app</a>' +
    '<a class="btn ghost" href="/download">⬇ Get Super AI</a>' +
    '</div>' +
    '<form class="search" onsubmit="return doSearch(event)">' +
    '<span class="si">🔍</span>' +
    '<input id="q" placeholder="Search apps, games, tools…" value="' + esc(list.q || "") + '">' +
    '</form>' +
    '</header>' +
    '<section class="feat">' +
    '<a class="feat-card" href="/store/app/app_superai">' +
    '<div class="feat-ic">🤖</div>' +
    '<div class="feat-tx"><div class="feat-tag">FLAGSHIP</div>' +
    '<h3>Super AI</h3><p>Free all-in-one AI: chat, image generation, downloaders &amp; a creative studio.</p></div>' +
    '<span class="feat-btn">Get it ↓</span></a>' +
    '</section>' +
    '<section class="browse">' +
    '<div class="sec-head"><h2>Browse apps</h2><div class="cats">' + chips + '</div></div>' +
    grid +
    '</section>' +
    '<script>function go(u){location=u;}function doSearch(e){e.preventDefault();var v=document.getElementById("q").value.trim();location="/store?q="+encodeURIComponent(v);}function enc(s){return encodeURIComponent(s);}</script>';
...
export function appCard(a) {
  ...
  return '<a class="card" href="/store/app/' + enc(a.id) + '">' +
    '<div class="card-ic"><img src="' + icon + '" alt="" loading="lazy" onerror="this.src=\'/api/asset/public/aaa-store-logo.png\'"></div>' +
    '<div class="card-cat">' + esc(a.category || "Other") + '</div>' +
    '<h3>' + esc(a.name) + '</h3>' +
    '<p>' + esc(a.short_desc || "") + '</p>' +
    '<div class="card-meta">⬇ ' + (a.downloads || 0) + ' · v' + esc(a.version || "?") + '</div>' +
    '</a>';
}
...
export function detailPage(a, user, ratings, versions) {
  ...
  const verifiedBadge = a.status === "approved" ? '<span class="badge ok">✓ Verified</span>' : '';
  const reviews = (r.reviews || []).slice(0, 6).map((rv) => {
    const s = "★★★★★".slice(0, rv.stars || 5);
    return '<div class="review"><div class="r-stars">' + s + '</div>' +
      (rv.review ? '<p>' + esc(rv.review) + '</p>' : '') +
      '<div class="r-uid">' + esc(rv.uid || "anon") + '</div></div>';
  }).join('');
  const rateForm = user
    ? '<form id="rf" class="rate" onsubmit="return postRate(event)">' +
      ...
      '<p id="rmsg" class="rmsg"></p></form>' +
      '<script>var _st=5;function setStars(n){_st=n;var b=document.querySelectorAll("#stars button");b.forEach(function(x,i){x.classList.toggle("on",i<n);});}' +
      'setStars(5);function postRate(e){e.preventDefault();var rv=document.getElementById("rv").value;fetch("/api/store/apps/' + enc(a.id) + '/rate",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({stars:_st,review:rv})}).then(function(r){return r.json();}).then(function(j){document.getElementById("rmsg").textContent=j.ok?"Thanks!":"Failed";setTimeout(function(){location.reload();},800);});return false;}</script>'
    : '<p class="muted center">Sign in to rate this app.</p>';
  const body =
    '<article class="detail">' +
    '<a class="back" href="/store">‹ Back</a>' +
    '<header class="d-head">' +
    '<img class="d-ic" src="' + icon + '" alt="" onerror="this.src=\'/api/asset/public/aaa-store-logo.png\'">' +
    '<div><h1>' + esc(a.name) + '</h1><div class="d-sub">' + esc(a.category || "Other") + ' · v' + esc(a.version || "?") + ' ' + verifiedBadge + '</div>' +
    '<div class="d-stars">' + stars + ' <span class="muted">' + r.avg + ' · ' + r.count + ' rating' + (r.count === 1 ? '' : 's') + '</span></div></div>' +
    '<a class="btn dl" href="' + (user ? '/store/apks/' + enc((a.apk_r2_key || '').replace(/^store\/apks\//, '').replace(/\.apk$/, '')) + '.apk' : '/store/login?next=' + enc('/store/apks/' + enc((a.apk_r2_key || '').replace(/^store\/apks\//, '').replace(/\.apk$/, '')) + '.apk')) + '">⬇ Download APK</a>' +
    '</header>' +
    '<p class="d-desc">' + esc(a.long_desc || a.short_desc || "No description provided.") + '</p>' +
    '<section class="panel"><h3>Rate &amp; review</h3>' + rateForm + '</section>' +
    (reviews ? '<section class="panel"><h3>Reviews</h3><div class="reviews">' + (reviews || '<p class="muted">No reviews yet.</p>') + '</div></section>' : '') +
    '</article>';
...
export function downloadPage(available, versionName, sizeLabel, stats, changelog, qr) {
  ...
  const body =
    ...
    '<h1>Get <span class="g">Super AI</span></h1>' +
    '<p class="lede">The free all-in-one AI app for Android. Chat, create, download — all in one tap.</p>' +
    (available
      ? '<div class="dl-card">' +
        '<div class="dl-info"><div class="dl-ver">v' + esc(versionName || "2.2.14") + (sizeLabel ? ' · ' + esc(sizeLabel) : '') + '</div>' +
        '<div class="dl-stats"><span>⬇ ' + (st.downloads || 0) + ' downloads</span><span>⭐ ' + (st.stars || 0) + ' rating</span></div></div>' +
        '<div class="dl-qr">' + (qr ? '<img src="' + qr + '" alt="QR">' : '') + '<span>Scan to install</span></div>' +
        '</div>' +
    ...
    (changelog ? '<details class="chg"><summary>What\'s new</summary><p>' + esc(changelog) + '</p></details>' : '') +
    '</header>' +
    '<section class="steps">' +
...
function step(n, t, d) {
  return '<div class="step"><div class="n">' + n + '</div><div><h4>' + esc(t) + '</h4><p>' + esc(d) + '</p></div></div>';
}
...
export function loginPage(opts) {
  ...
  const body =
    ...
    'data-telegram-login="' + esc(bot) + '" data-size="large" data-userpic="false" data-radius="16" ' +
    'data-auth-url="' + esc(verifyUrl) + '" data-request-access="write"></script></div>' +
    '<div class="div">or</div>' +
    '<label>Telegram link code</label>' +
    ...
    '<button class="btn" id="codeBtn">Verify code</button>' +
    ...
    'fetch("/api/store/login",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code:c})}).then(function(r){return r.json();}).then(function(j){if(j.ok){saveSess(j.token);location.href=NEXT;}else{document.getElementById("msg").textContent=j.error||"Invalid code.";}});});' +
    '</script></div>';
...
export function uploadPage(user) {
  ...
  const body =
    ...
    fld("Category", '<select id="category">' + (typeof STORE_CATEGORIES !== "undefined" ? STORE_CATEGORIES : ["Game", "Tools", "Social", "Productivity", "Entertainment", "Education", "Finance", "Other"]).map((c) => '<option>' + esc(c) + '</option>').join('') + '</select>') +
    ...
    '<p id="umsg" class="msg"></p>' +
    '</form>' +
    '<script>function submitApp(e){e.preventDefault();var d={name:name.value,package_name:package_name.value,category:category.value,version:version.value,short_desc:short_desc.value,long_desc:long_desc.value,apk_url:apk_url.value,icon_url:icon_url.value};fetch("/api/store/apps",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(d)}).then(function(r){return r.json();}).then(function(j){document.getElementById("umsg").textContent=j.ok?"Submitted! Awaiting review.":("Error: "+(j.error||"unknown"));});return false;}</script>' +
    '</div></div>';
...
export function profilePage(user) {
  ...
  const avatar = user.photo_url
    ? '<img class="p-av" src="' + esc(user.photo_url) + '" alt="">'
    : '<div class="p-av p-av--i">' + esc((user.display_name || user.tg_username || "U").slice(0, 1).toUpperCase()) + '</div>';
  const tg = user.telegram_id;
  const rows = [
    ["Telegram ID", tg ? esc(String(tg)) : '<span class="muted">not linked</span>'],
    ["Account UID", esc(user.uid || "")],
    ["Username", user.tg_username ? "@" + esc(user.tg_username) : '<span class="muted">—</span>'],
    ["First name", user.first_name ? esc(user.first_name) : '<span class="muted">—</span>'],
    ["Last name", user.last_name ? esc(user.last_name) : '<span class="muted">—</span>'],
    ["Premium", user.is_premium ? "★ Yes" : "No"],
    ["Language", user.language_code ? esc(user.language_code) : '<span class="muted">—</span>'],
    ["Phone", user.phone ? esc(user.phone) : '<span class="muted">—</span>'],
  ];
  const body =
    '<div class="auth wide"><div class="auth-card">' +
    '<div class="p-head">' + avatar +
    '<div><div class="p-name">' + esc(user.display_name || user.tg_username || user.uid || "User") + (user.is_premium ? ' <span class="star">★</span>' : '') + '</div>' +
    '<div class="p-sub">' + (user.tg_username ? "@" + esc(user.tg_username) : esc(user.uid || "")) + '</div></div></div>' +
    '<div class="kv">' + rows.map((r) => '<div class="kv-row"><div class="k">' + r[0] + '</div><div class="v">' + r[1] + '</div></div>').join('') + '</div>' +
    '<a class="btn ghost" href="/store" style="margin-top:18px">← Back to store</a>' +
    '</div></div>';
  return shell("Your profile — AAA App Store", body, user);
}

// ===========================================================================
// Design tokens + base CSS (the actual "from scratch" visual system)
// ===========================================================================
function designTokens() {
  return ':root{color-scheme:dark;' +
    ...
    '--glow:rgba(124,77,255,.45);--radius:20px;--maxw:1120px;' +
    '--shadow:0 18px 50px rgba(0,0,0,.45);}' +
    'html[data-theme="light"]{--bg:#f5f6fc;--bg2:#ffffff;--fg:#16162a;--muted:#5d5d78;--card:rgba(20,20,50,.04);' +
    '--card2:rgba(20,20,50,.07);--border:rgba(20,20,50,.12);--input:rgba(20,20,50,.05);' +
    '--shadow:0 18px 50px rgba(80,60,160,.12);}' +
    '*{box-sizing:border-box;margin:0;padding:0}' +
    'body{font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:var(--bg);' +
    'color:var(--fg);line-height:1.6;-webkit-font-smoothing:antialiased;transition:background .25s,color .25s;overflow-x:hidden}' +
...
function baseCss() {
  return '' +
    ...
    '.icn:hover{border-color:var(--brand);transform:translateY(-1px)}' +
    '.fb{color:#9db4ff;font-weight:900;text-decoration:none}' +
    ...
    '.pill:hover{transform:translateY(-2px);box-shadow:0 10px 26px var(--glow)}' +
    '.pill.ghost{background:var(--input);border:1px solid var(--border);color:var(--fg)}' +
    ...
    // Featured
    '.feat{margin:8px auto 0;max-width:680px}' +
    '.feat-card{display:flex;align-items:center;gap:16px;background:linear-gradient(135deg,rgba(124,77,255,.16),rgba(255,77,157,.12));border:1px solid rgba(124,77,255,.32);border-radius:var(--radius);padding:18px 22px;transition:.2s}' +
    '.feat-card:hover{transform:translateY(-3px);border-color:var(--brand);box-shadow:var(--shadow)}' +
    '.feat-ic{width:58px;height:58px;border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:1.8rem;background:var(--grad);box-shadow:0 8px 22px var(--glow)}' +
    '.feat-tx{flex:1;text-align:left}.feat-tag{font-size:.66rem;font-weight:800;letter-spacing:.18em;color:#c4b5ff}.feat-tx h3{font-size:1.15rem}.feat-tx p{color:var(--muted);font-size:.86rem}' +
    '.feat-btn{background:var(--grad);color:#fff;font-weight:800;padding:10px 18px;border-radius:50px;font-size:.85rem;white-space:nowrap}' +
    // Browse
    '.browse{padding:40px 0 20px}.sec-head{display:flex;flex-direction:column;gap:16px;margin-bottom:8px}' +
    '.sec-head h2{font-size:1.6rem;font-weight:900}.cats{display:flex;gap:8px;flex-wrap:wrap}' +
    '.cat{background:var(--input);border:1px solid var(--border);color:var(--fg);padding:8px 16px;border-radius:50px;font-size:.85rem;cursor:pointer;transition:.16s}' +
    '.cat:hover{border-color:var(--brand)}.cat.on{background:var(--grad);border-color:transparent;color:#fff;font-weight:700}' +
    ...
    // Detail
    '.detail{max-width:760px;margin:24px auto 40px}.back{color:var(--muted);font-weight:600}.back:hover{color:var(--fg)}' +
    ...
    '.d-head .dl{margin-left:auto}' +
    '.d-desc{color:var(--muted);margin:18px 0;white-space:pre-wrap}' +
    '.panel{background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:22px;margin:18px 0}' +
    '.panel h3{font-size:1.1rem;margin-bottom:14px}' +
    '.rate .stars{font-size:1.6rem;color:#3a3a4a;margin-bottom:10px}.rate .stars button{background:none;border:none;cursor:pointer;color:#3a3a4a;font-size:1.6rem;transition:.12s}.rate .stars button.on{color:#ffd54a}' +
    '.rate textarea{width:100%;background:var(--input);border:1px solid var(--border);color:var(--fg);border-radius:12px;padding:12px;font-family:inherit;resize:vertical}' +
    ...
    '.up input:focus,.up textarea:focus,.up select:focus{border-color:var(--brand)}' +
    // Profile
...
function fld(label, control) { return '<label>' + esc(label) + '</label>' + control; }
